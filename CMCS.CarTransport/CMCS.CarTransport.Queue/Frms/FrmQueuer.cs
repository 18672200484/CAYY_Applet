...
namespace CMCS.CarTransport.Queue.Frms
{
    public partial class FrmQueuer : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        bool SaveSaleFuelTransport()
        {
            ...
            //{
            //    MessageBoxEx.Show("请选择销售煤订单", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //    return false;
            //}
            //if (StorageNames.IndexOf(cmbCoalProduct.SelectedItem.ToString()) < 0 && MessageBoxEx.Show(cmbCoalProduct.SelectedItem.ToString() + "成品仓不再销售订单内，确定选择？", "操作提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
            //{
            //    return false;
            //}
            try
