...
namespace CMCS.CarTransport.Queue.Frms.Transport.BuyFuelTransport
{
	public partial class FrmBuyFuelTransport_Oper : DevComponents.DotNetBar.Metro.MetroForm
	{
		...
		/// 车辆
		/// </summary>
		public CmcsAutotruck CmcsAutotruck
		{
			get { return cmcsAutotruck; }
			set { cmcsAutotruck = value; }
		}
		...
		private void btnSubmit_Click(object sender, EventArgs e)
		{
			if (this.EditMode == eEditMode.修改)
			{
				...
				if (CmcsBuyFuelTransport.CarNumber != txt_CarNumber.Text)//车号发生改变
				{
					CmcsBuyFuelTransport.AutotruckId = CmcsAutotruck.Id;
					CmcsUnFinishTransport unfinishTransportAutotruck = Dbers.GetInstance().SelfDber.Entity<CmcsUnFinishTransport>(" where AutotruckId=:AutotruckId and CarType=:CarType", new { AutotruckId = CmcsAutotruck.Id, CarType = eCarType.入厂煤.ToString() });
					if (unfinishTransportAutotruck != null)
						Dbers.GetInstance().SelfDber.Delete<CmcsUnFinishTransport>(unfinishTransportAutotruck.Id);
					CmcsBuyFuelTransport.AutotruckId = CmcsAutotruck.Id;
				}
				...
				if (CmcsBuyFuelTransport.TareWeight == 0)
				{
					CmcsBuyFuelTransport.SuttleWeight = 0;
				}
				else
				{
					CmcsBuyFuelTransport.SuttleWeight = CmcsBuyFuelTransport.GrossWeight - CmcsBuyFuelTransport.TareWeight;
				}
				...
				CmcsBuyFuelTransport.HeavyWeight = cmbHeavyWeight.Text;
				CmcsBuyFuelTransport.ShipName = txtShipNumber.Text;
				CmcsBuyFuelTransport.IsSyncBatch = 0;

				CmcsUnFinishTransport unfinishTransport = Dbers.GetInstance().SelfDber.Entity<CmcsUnFinishTransport>(" where AutotruckId=:AutotruckId and CarType=:CarType", new { AutotruckId = CmcsBuyFuelTransport.AutotruckId, CarType = eCarType.入厂煤.ToString() });
