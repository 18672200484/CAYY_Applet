...
namespace CMCS.CarTransport.Queue.Frms.BaseInfo.CamareInfo
{
    public partial class FrmCamare_List : MetroForm
    {
        ...
        void ProcessFromRequest(eEditMode editMode)
        {
            switch (editMode)
            {
                case eEditMode.新增:
                    ...
                    dbi_Sequence.Value = commonDAO.GetCameraSort();
                    break;
                case eEditMode.修改:
                    CurrEditMode = editMode;
                    InitObjectInfo();
        ...
        void DelTreeNode()
        {
            if (this.SelCamera.Id == "-1") { MessageBoxEx.Show("根节点不允许删除!", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        ...
        private bool ValidatePage()
        {
            ...
            {
                MessageBoxEx.Show("摄像头名称不能为空!", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
            if (commonDAO.IsExistCameraName(txt_CameraName.Text, SelCamera.Id))
            {
                MessageBoxEx.Show("已有相同摄像头名称!", "操作提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return false;
            }
            return true;
        }
