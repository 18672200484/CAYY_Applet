...
namespace CMCS.Common.Utilities
{
	...
	public class AssayCalcUtil
	{
		...
		/// <returns></returns>
		public static decimal mathRount(decimal value, int digit)
		{
			var ratio = Convert.ToDecimal(Math.Pow(10, digit));
			var _value = value * ratio;
			var integer = Math.Floor(_value);
			var mode = Math.Floor(value * Convert.ToDecimal(Math.Pow(10, digit + 1)) % 10);
			if (mode > 5m)
			{
			...
			{
				var mode1 = Convert.ToInt32(value.ToString().Split('.')[1].Substring(1, 1));
				if (mode1 == 0)
					return integer / ratio;
				else if (mode1 % 2 == 0)
				{
		...
		public static decimal CalcAvgValue(IList<decimal> values, int decimals = 2)
		{
			...
			return mathRount(sum / value1.Count() * 1m, decimals);
		}
