...
namespace CMCS.Common.DAO
{
	...
	public class CommonDAO
	{
		...
		{
			CmcsUser user = SelfDber.Entity<CmcsUser>("where UserName=:UserName", new { UserAccount = Account });
		...
		/// <returns></returns>
		public bool SaveSysMessage(String msgCode, String msgContent, String windowTitle = "操作提示", String msgButton = "查看|取消", bool msgWarnType = true, bool isAutoClose = false, String msgParam = "")
		{
		...
		/// </summary>
		/// <returns></returns>
		public int GetFuelKindSort()
		{
			return Dbers.GetInstance().SelfDber.Entities<CmcsFuelKind>().Max(a => a.Sort) + 1;
		}
		...
		/// <param name="isUse">状态值</param>
		public void UpdateFuelKindChildsIsUse(string parentId, int isUse)
		{
			List<CmcsFuelKind> list = Dbers.GetInstance().SelfDber.Entities<CmcsFuelKind>("where ParentId=:ParentId", new { ParentId = parentId });
			if (list.Count > 0)
			{
				foreach (var item in list)
				{
					//更新子节点
					UpdateFuelKindChildsIsUse(item.Id, isUse);
		...
		{
			return Dbers.GetInstance().SelfDber.Entities<CmcsMine>().Max(a => a.Sort) + 1;
		}
		...
		/// <param name="parentId">父节点id</param>
		/// <param name="isUse">状态值</param>
		public void UpdateMineChildsIsUse(string parentId, int isUse)
		{
			List<CmcsMine> list = Dbers.GetInstance().SelfDber.Entities<CmcsMine>("where ParentId=:ParentId", new { ParentId = parentId });
			if (list.Count > 0)
			{
				foreach (var item in list)
				{
					//更新子节点
					UpdateMineChildsIsUse(item.Id, isUse);
		...
		{
			return Dbers.GetInstance().SelfDber.Entities<CmcsGoodsType>().Max(a => a.OrderNumber) + 1;
		}
		...
		/// <param name="parentId">父节点id</param>
		/// <param name="isUse">状态值</param>
		public void UpdateGoodsChildsIsUse(string parentId, int isUse)
		{
			List<CmcsGoodsType> list = Dbers.GetInstance().SelfDber.Entities<CmcsGoodsType>("where ParentId=:ParentId", new { ParentId = parentId });
			if (list.Count > 0)
			{
				foreach (var item in list)
				{
					//更新子节点
					UpdateGoodsChildsIsUse(item.Id, isUse);
		...
		{
			return Dbers.GetInstance().SelfDber.Entities<CmcsCamare>().Max(a => a.Sort) + 1;
		}
		...
		/// <returns></returns>
		public double GetTrainRateLoadByTrainType(string trainType)
		{
