...
namespace CMCS.Common.DAO
{
	...
	public class BeltSamplerDAO
	{
		...
		/// <param name="interfaceType">接口类型</param>
		/// <returns></returns>
		public List<InfBeltSamplePlanDetail> GetWaitForSyncBeltSamplePlanDetail(string machineCode)
		{
			return Dbers.GetInstance().SelfDber.Entities<InfBeltSamplePlanDetail>("where MchineCode=:MchineCode and SyncFlag=0", new { MchineCode = machineCode });
		}
