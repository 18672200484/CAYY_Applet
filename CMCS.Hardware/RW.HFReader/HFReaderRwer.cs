...
namespace RW.HFReader
{
	public class HFReaderRwer
	{
		...
		/// <param name="readerAddr">读卡器地址</param>
		/// <returns></returns>
		public bool OpenComPort(int port, string readAddr = "FF")
		{
			readerAddr = Convert.ToByte(readAddr, 16); // $FF;

			int openresult = StaticClassReaderB.OpenComPort(port, ref readerAddr, ref portFrmIndex);
			if (openresult == 0)
			{
				if (OpenRF())
				{
		...
		public bool CloseComPort()
		{
			if (portFrmIndex > 1023)
			{
				int fCmdRet = StaticClassReaderB.CloseSpecComPort(portFrmIndex);
				if (fCmdRet == 0)
				{
					portFrmIndex = -1;
					SetStatus(false);
