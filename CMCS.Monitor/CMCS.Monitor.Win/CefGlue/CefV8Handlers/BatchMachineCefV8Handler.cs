...
namespace CMCS.Monitor.Win.CefGlue
{
	...
	/// 合样归批监控 CefV8Handler
	/// </summary>
	public class BatchMachineCefV8Handler : CefV8Handler
	{
		...
		protected override bool Execute(string name, CefV8Value obj, CefV8Value[] arguments, out CefV8Value returnValue, out string exception)
		{
			...
			switch (name)
			{
				...
				//  打开合样归批报警信息
				case "ErrorInfo":
					CefProcessMessage cefMsg = CefProcessMessage.Create("BatchMachineErrorInfo");
				...
				//  倒料命令
				case "SendDLCMD":
					cefMsg = CefProcessMessage.Create("BatchMachineSendDLCMD");
					cefMsg.Arguments.SetSize(0);
					cefMsg.Arguments.SetString(0, arguments[0].GetStringValue());
					CefV8Context.GetCurrentContext().GetBrowser().SendProcessMessage(CefProcessId.Browser, cefMsg);
					break;
				//  倒料命令
				case "SendGPCMD":
					cefMsg = CefProcessMessage.Create("BatchMachineSendGPCMD");
					cefMsg.Arguments.SetSize(0);
					cefMsg.Arguments.SetString(0, arguments[0].GetStringValue());
					CefV8Context.GetCurrentContext().GetBrowser().SendProcessMessage(CefProcessId.Browser, cefMsg);
					break;
				//  卸车刷卡
				case "UnloadSwip":
					commonDAO.SendAppRemoteControlCmd(GlobalVars.MachineCode_HYGPJ_1, "卸车刷卡", "1");
					cefMsg1 = CefProcessMessage.Create("SaveOperationLog");
					cefMsg1.Arguments.SetSize(0);
					cefMsg1.Arguments.SetString(0, "设置" + GlobalVars.MachineCode_HYGPJ_1 + "卸车刷卡");
					CefV8Context.GetCurrentContext().GetBrowser().SendProcessMessage(CefProcessId.Browser, cefMsg1);
					break;
				//  装车刷卡
				case "TruckUnloadSwip":
					commonDAO.SendAppRemoteControlCmd(GlobalVars.MachineCode_HYGPJ_1, "装车刷卡", "1");
					cefMsg1 = CefProcessMessage.Create("SaveOperationLog");
					cefMsg1.Arguments.SetSize(0);
					cefMsg1.Arguments.SetString(0, "设置" + GlobalVars.MachineCode_HYGPJ_1 + "装车刷卡");
					CefV8Context.GetCurrentContext().GetBrowser().SendProcessMessage(CefProcessId.Browser, cefMsg1);
					break;
				//  历史故障
				case "FaultRecord":
