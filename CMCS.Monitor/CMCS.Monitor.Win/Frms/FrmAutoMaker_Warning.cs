...
{
	public partial class FrmAutoMaker_Warning : DevComponents.DotNetBar.Metro.MetroForm
	{
		/// <summary>
		/// 窗体唯一标识符
		/// </summary>
		public static string UniqueKey = "FrmAutoMaker_Warning";

		public FrmAutoMaker_Warning()
		{
			InitializeComponent();
		}
		...
		private void FrmAutoMaker_Warning_Load(object sender, EventArgs e)
		{
			
		}

		private void FrmAutoMaker_Warning_Shown(object sender, EventArgs e)
		{
			InitForm();
		...
		private void FrmAutoMaker_Warning_FormClosing(object sender, FormClosingEventArgs e)
		{
		...
		{
			string sql = string.Format(@"select a.signalprefix,a.signalname,a.updatetime from cmcstbsignaldata a where a.signalprefix='#1全自动制样机' and a.signalname like '%AL_%' and a.signalvalue='1'
										");
			...
			for (int i = 0; i < dt.Rows.Count; i++)
			{
				...
				entity.Remark = dt.Rows[i]["signalname"].ToString();
				list.Add(entity);
			}
