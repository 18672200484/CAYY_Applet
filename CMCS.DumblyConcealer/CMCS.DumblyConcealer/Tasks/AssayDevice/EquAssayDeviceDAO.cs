...
namespace CMCS.DumblyConcealer.Tasks.AssayDevice
{
	public class EquAssayDeviceDAO
	{
		...
		private CmcsFuelQuality getHyCyMz(CmcsRCAssay entity, CmcsFuelQuality Quality)
		{
			...
			//SumAad(entity.BillNumber);
			//SumVad(entity.BillNumber);
		...
		{
			if (commonDAO.SelfDber.Count<CmcsMoistureAssay>("where SampleNumber=:assayBillnumber and IsEffective=1 and WaterType='全水分'", new { assayBillnumber = assayBillnumber }) > 0)
				return;
			IList<CmcsMoistureAssay> mtAssay = Dbers.GetInstance().SelfDber.TopEntities<CmcsMoistureAssay>(4, "where SAMPLENUMBER=:assayBillnumber and IsEffective=0 and WaterType='全水分' order by AssayTime", new { assayBillnumber = assayBillnumber }).ToList();
		...
		public CmcsFuelQuality getQuality(CmcsFuelQuality Quality)
		{
			...
			//	Quality.QJ = Math.Round((((qbj * 1000 - 206 * Quality.Had) * ((100 - Quality.Mar) / (100 - Quality.Mad)) - 23 * Quality.Mar) / 1000), 3);
